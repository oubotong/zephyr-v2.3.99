...
extern int z_impl_can_send(struct device * dev, const struct zcan_frame * msg, k_timeout_t timeout, can_tx_callback_t callback_isr, void * callback_arg);
static inline int can_send(struct device * dev, const struct zcan_frame * msg, k_timeout_t timeout, can_tx_callback_t callback_isr, void * callback_arg)
{
...
extern int z_impl_can_configure(struct device * dev, enum can_mode mode, uint32_t bitrate);
static inline int can_configure(struct device * dev, enum can_mode mode, uint32_t bitrate)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
...
extern int z_impl_can_recover(struct device * dev, k_timeout_t timeout);
static inline int can_recover(struct device * dev, k_timeout_t timeout)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		union { struct { uintptr_t lo, hi; } split; k_timeout_t val; } parm0;
